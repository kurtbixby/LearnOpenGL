...
inline unsigned int load_texture(std::string texture_path, const GLenum source_format, const GLenum wrap_type)
{
	...
	load_texture_file(texture_file, source_format, GL_TEXTURE_2D);
...
inline unsigned int load_cubemap(std::vector<std::string> texturePaths)
{
	...
	for (int i = 0; i < 6; i++)
	{
		...
		load_texture_file(texture_file, GL_RGB, GL_TEXTURE_CUBE_MAP_POSITIVE_X + i);
	}
