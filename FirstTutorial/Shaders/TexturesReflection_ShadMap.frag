...
bool shadow_calculation(vec4 fragLightPos, sampler2D lightShadMap, vec3 lightDir)
{
    ...
    float shadMapDepth = texture(lightShadMap, projCoords.xy).r;
    return (projCoords.z - bias) > shadMapDepth ? true : false;
}

// Should include a shadow map variable in the future for different lights
vec3 global_lighting(Light light, sampler2D lightShadMap, vec4 fragLightPos, vec3 diffuseValue, vec3 specularValue)
{
    if (shadow_calculation(fragLightPos, lightShadMap, light.direction))
    {
        return vec3(0.0f);
    }
...
void main()
{
//    FragColor = vec4(fs_in.FragLightPos.xyz / fs_in.FragLightPos.w, 1.0f);
	...
	for (i = 0; i < DIR_LIGHTS; i++)
	{
		...
		global += global_lighting(lights[i], shadowMap, fs_in.FragLightPos, diffuseValue, specularValue);
	}
