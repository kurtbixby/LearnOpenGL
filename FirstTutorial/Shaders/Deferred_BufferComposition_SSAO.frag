...
#define FragSSAO texture(ssaoTexture, TexCoords).r
...
vec3 global_lighting(Light light, sampler2D lightShadMap, mat4 lightSpaceMatrix)
{
    ...
    vec3 ambient = FragSSAO * AMBIENT_COEF * (light.ambient * FragColorDiffuse);
...
//    light.specular = pointLight.specular;
    ...
    vec3 ambient = FragSSAO * AMBIENT_COEF * (pointLight.ambient * FragColorDiffuse);
...
vec3 spot_lighting(SpotLight spotLight)
{
    ...
    if (fragDotProd > spotLight.outerCutoff)
    {
        ...
        vec3 ambient = FragSSAO * AMBIENT_COEF * (spotLight.ambient * FragColorDiffuse);
